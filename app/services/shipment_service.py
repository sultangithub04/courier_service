...
def list_shipments(
    ...
    if user.role == Role.USER:
        q = q.filter(Shipment.sender_id == user.id)
    ...
    if search:
        ...
        q = q.filter(
            or_(
                ...
                Shipment.receiver_name.ilike(term),
                Shipment.receiver_phone.ilike(term),
                Shipment.destination.ilike(term),
                Shipment.origin.ilike(term),
            )
        )
    if status:
        q = q.filter(Shipment.shipment_status == status)
    if payment_status:
        q = q.filter(Shipment.payment_status == payment_status)
    if city:
        q = q.filter(Shipment.destination.ilike(f"%{city}%"))
    if delivery_agent_id:
        q = q.filter(Shipment.delivery_agent_id == delivery_agent_id)
    if start_date:
        q = q.filter(Shipment.created_at >= start_date)
    if end_date:
        q = q.filter(Shipment.created_at <= end_date)
    col = SORTS.get(sort_by, Shipment.created_at)
